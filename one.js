...
/*
Polygon can be made by multiple Tria.
Polygon shape's center is offset by 3D vector from the origin
Polygon shape's plane is perpendicular to the tranlation vector
Polygon shape takes edge amount and corner center distance as radius
*/
Class Polygon extends Origin {
	constructor(edge, radius, translateX, translateY, translateZ, originx, originy, originz){
		super(originx, originy, originz);
		this.edgeAmount = edge;
		this.radius = radius;
		this.transX = translateX;
		this.transY = translateY;
		this.transZ = translateZ;
	}
	get centerPoint() {
		return [this.originX + this.transX, this.originY + this.transY, this.originZ + this.transZ];
	}
	get translateVector() {
		return [this.transX, this.transY, this.transZ];
	}
	onPlane(x, y, z){
		let a = this.translateVector[0];
		let b = this.translateVector[1];
		let c = this.translateVector[2];
		let x0 = this.centerPoint[0];
		let y0 = this.centerPoint[1];
		let z0 = this.centerPoint[2];
		if (a*x0 + b*y0 + c*z0 === 0){
			return true;
		}
		else {
	...
	get intersectPlane(){
		if (this.translateVector[0] !== 0){
			return "X"
		}
		elseif (this.translateVector[1] !== 0){
			return "Y"
		}
		elseif (this.translateVector[2] !== 0){
			return "Y"
		}
	}
